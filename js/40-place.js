...
function scoreItem(it, c) {
  ...
  /* 5. 단지 경쟁력 — 세대수·역세권 (연식은 점수 제외, 표시만)
     v43.4 확정: K-apt 실제 세대수·역세권으로 재검증
       (전국 104곳 · 단지 12,934곳 · 단지명 매칭 59% · 반기 진입)
     - 역세권 : 단독 IC 10년 +0.129 로 가장 강했다. 비중 .25 → .55
                구간별 초과수익은 5분 이내만 +3.1%p 이고 6분부터 전부 음수 →
                기존의 완만한 계단(100/88/72/55)을 '5분 절벽' 형태로 교체
     - 세대수 : 단독 IC 10년 +0.116. 500~999(+1.3%p)와 1000~1999(+1.5%p)가 사실상 동일하고
                2000 이상만 +8.0%p 로 튀어, 중간 계단을 없애고 2000 절벽으로 교체. 비중 .55 → .45
     - 연식   : 신축 우대 곡선 IC -0.114, 구축 우대로 바꿔도 -0.003. 어느 방향도 값을 못 해
                배점에서 제외(0%)하고 화면에는 계속 표시한다
     ※ 지난 v43.3 의 세대수 IC(+0.128)는 거래건수를 세대수 대용으로 쓴 값이었고,
       실제 세대수와의 상관이 0.30 에 그쳐 과대 추정이었다. 실측치로 교체했다. */
  ...
  var s5 = pHh * .45 + pWk * .55;
  ax.push({ k: '단지 경쟁력', w: .15, s: s5,
    d: (hh ? n0(hh) + '세대' + (hh >= 2000 ? ' (초대형)' : hh >= 500 ? '' : ' (소규모)') : '세대수 미상') + ' · ' +
       (walk == null ? '역 정보 없음' : '도보 ' + walk + '분' + (walk <= 5 ? ' (초역세권)' : '')) +
       ' · ' + (age == null ? '연식 미상' : age + '년차 (점수 미반영)') });
