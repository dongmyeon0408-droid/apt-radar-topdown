...
function recTopApts() {
  ...
  pool(tasks, 8).then(function (res) {
      ...
      targets.forEach(function (x) {
        (byReg[x.r.code] || []).forEach(function (g) {
          ...
          window.__ENRICH[key] = {
            ...
            matchedNorm: m.matchedNorm || null,
            normQuery: m.normQuery || null,
            matchMethod: m.method || 'none',
            matchScore: m.matchScore == null ? null : m.matchScore,
            runnerUp: m.runnerUp == null ? null : m.runnerUp,
            gap: m.gap == null ? null : m.gap,
            failReason: m.failReason || null,
            kaptListSize: m.kaptListSize == null ? null : m.kaptListSize,
            /* coverage 분해용 원자료 */
    ...
    targets.forEach(function (x) {
      ...
      /* v52.0 — 가격 하한 70% 를 함께 건다.
         뒤처짐만 반영했더니 «5년간 -20% 빠진 단지»가 1위로 올라왔다.
         많이 빠진 단지는 «따라올 차례»가 아니라 «빠질 이유가 있는 것»일 때가 많다.
         검증(bt-mom2, 159,225회):
           역순위30%만        +34.9%p · 1위가 최상단의 68% · 고른 단지 직전상승 +16%
...
/* best = 지역은 대출 기준 · 단지는 전세 허용 · 뒤처짐 30%(직전 5년) + 가격 하한 70%
          (bt-mom2 전략 ⑧, 159,225회 검증 — 9종 중 1위)
   loan = 대출만 · 뒤처짐 미반영 (bt-area, 99,598회)
   gap  = 전세 끼고만 (지역까지 전세 기준) · first = 생애최초 LTV 70% */
var BT = {
  36:  { best:[8.4,16.5,36.5,58.7],  loan:[6.8,15.2,32.5,54.5],  first:[7.7,16.0,33.6,57.4],
         gap:[0.3,6.9,19.4,39.5],  win:[63,80,98,96],  winF:[65,80,98,96],  bench:68.1 },
  46:  { best:[13.1,29.8,45.5,95.2], loan:[10.2,24.6,41.2,90.6], first:[11.9,28.5,45.3,104.4],
         gap:[5.5,13.5,26.1,53.9], win:[68,86,96,98],  winF:[68,87,96,98],  bench:65.2 },
  59:  { best:[13.5,28.9,49.9,82.6], loan:[13.3,24.9,47.4,76.2], first:[16.2,29.9,54.0,87.4],
         gap:[5.5,13.1,27.5,44.2], win:[70,83,95,96],  winF:[73,86,97,96],  bench:60.9 },
  84:  { best:[8.6,20.0,39.5,68.1],  loan:[7.9,18.9,38.1,58.4],  first:[10.4,23.5,44.5,70.7],
         gap:[2.9,7.5,16.4,21.0],  win:[63,76,95,98],  winF:[66,80,97,98],  bench:63.6 },
  101: { best:[6.3,11.1,22.1,45.4],  loan:[6.0,10.7,24.9,35.5],  first:[7.8,14.6,30.0,47.4],
...
function verifyBanner(c) {
  ...
  if (r.kind === 'gap') {
    ...
    msg = '지역은 <b>대출 매수 기준</b>으로 정하고, 단지는 <b>전세 허용 + 최근 5년 덜 오른 곳 30% 가점 + 가격 하한 70%</b>입니다. ' +
